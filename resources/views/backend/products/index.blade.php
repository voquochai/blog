		...
		<div class="card">
			<div class="card-header">
                <div class="card-widgets">
                    <a href="{{ route('admin.products.create', ['type'=>$type]) }}" class="btn btn-primary mr-2"> <i class="mdi mdi-plus"></i> Tạo mới</a>
                    <div class="dropdown">
                    ...
                    <table class="table table-centered">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                @if($config['image'])
                                <td> <a href="{{ route('admin.products.edit', ['id'=>$item->id, 'type'=>$type]) }}"> {!! ($item->image && file_exists(public_path(get_thumbnail($path.'/'.$item->image))) ) ? '<img src="'.asset(get_thumbnail('public/'.$path.'/'.$item->image).'?v='.time()).'" height="50" />':'' !!} </a> </td>
                                @endif
                                ...
                                <td>
                                    <a href="{{ route('admin.products.edit', ['id'=>$item->id, 'type'=>$type]) }}" class="btn btn-sm btn-primary">
